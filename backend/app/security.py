...
def get_current_user_flexible(
    ...
    """Auth that supports both Authorization header and ?token= query param.
    Used for file downloads and exports called via window.open()."""
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="无法验证凭据",
    )
    ...
    if user.locked_until and user.locked_until > datetime.now(timezone.utc):
        raise HTTPException(status_code=423, detail="账号已被锁定")
